...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateKey = async (clientToken, promoId) => {
        ...
        if (!response.ok) {
            throw new Error('کلید تولید نشد سرور فیلترشکن رو عوض کنید و دوباره امتحان کنید');
        }
    ...
    const copyToClipboard = (text) => {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            ...
            const textArea = document.createElement('ناحیه متنی');
            textArea.value = text;
            ...
            try {
                ...
                if (successful) {
                    copyStatus.classList.remove('پنهان شده است');
                    setTimeout(() => copyStatus.classList.add('hidden'), 2000);
                }
            } catch (err) {
